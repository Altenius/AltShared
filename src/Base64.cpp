...
std::string Base64::encode(const BYTE *buf, unsigned int bufLen)
{
    ...
    {
        // Read a group of three bytes (avoid buffer overrun by replacing with
        // 0)
        size_t index = i * 3;
        BYTE b3[3];
...
std::vector<BYTE> Base64::decode(std::string encoded_string)
{
    ...
    for (size_t i = 0; i < encoded_size; i += 4)
    {
        ...
        BYTE b4[4];
        b4[0] = (encoded_string[i + 0] <= 'z')
                    ? from_base64[encoded_string[i + 0]]
                    : 0xff;
        b4[1] = (encoded_string[i + 1] <= 'z')
                    ? from_base64[encoded_string[i + 1]]
                    : 0xff;
        b4[2] = (encoded_string[i + 2] <= 'z')
                    ? from_base64[encoded_string[i + 2]]
                    : 0xff;
        b4[3] = (encoded_string[i + 3] <= 'z')
                    ? from_base64[encoded_string[i + 3]]
                    : 0xff;
