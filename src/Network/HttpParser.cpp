...
HttpParser::HttpParser(HttpParser::Callbacks *callbacks)
    : callbacks_(callbacks), headerParser_(this), contentLength_(-1)
{
}
...
size_t HttpParser::parse(const char *data, size_t size)
{
    ...
    if (statusLine_.empty())
    {
        ...
        size_t headerParseConsumed =
            headerParser_.parse(buffer_.data(), buffer_.size());
        ...
        return statusParseConsumed + headerParseConsumed + bodyParseConsumed -
               inBuffer;
    }
    ...
    if (!headerParser_.done())
    {
        ...
        if (headerParser_.done())
        {
            ...
            size_t bodyParseConsumed = parseBody(data + headerParseConsumed,
                                                 size - headerParseConsumed);
            if (bodyParseConsumed == std::string::npos)
            {
                return std::string::npos;
            }
...
void HttpParser::headersFinished()
{
    ...
    transferEncodingParser_ = TransferEncodingParser::create(
        *this, transferEncoding_, contentLength_);
    if (transferEncodingParser_ == nullptr)
    {
        callbacks_->onError("unknown transfer encoding");
    }

    if (!contentEncoding_.empty() && contentEncoding_ != "identity")
    { // TODO: handling multiple content encodings
