...
void TcpSslContext::onWrite()
{
    ...
    {
        int ret = SslContext::send(
            reinterpret_cast<const unsigned char *>(writeBuffer_.data()),
            writeBuffer_.size());
        if (ret > 0)
        {
...
int TcpSslContext::send(const unsigned char *buf, size_t len)
{
    ...
    if (ret != len)
    {
        readBuffer_.append(reinterpret_cast<const char *>(buf) + ret,
                           len - ret);
    }
...
void TcpSslContext::onReceive(const char *buf, size_t len)
{
    ...
    char data[1024];
    int ret;
    while ((ret = SslContext::receive(reinterpret_cast<unsigned char *>(data),
                                      1024)) > 0)
    {
...
SecureTcpLink::SecureTcpLink(TcpLink::Callbacks &callbacks, bool client,
                             const char *hostname)
    : TcpLink(callbacks), ssl_(this)
{
