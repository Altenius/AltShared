...
TcpLinkPtr TcpLink::connect(const std::string &host, unsigned short port,
                            Callbacks &callbacks, bool ssl)
{
    ...
    bufferevent_setcb(link->bufferevent_, readCallback, writeCallback,
                      eventCallback, link.get());
    ...
    if (evutil_parse_sockaddr_port(
            host.c_str(), reinterpret_cast<sockaddr *>(&sa), &salen) == 0)
    {
        ...
        if (bufferevent_socket_connect(link->bufferevent_,
    ...
    else
    {
        if (bufferevent_socket_connect_hostname(
                link->bufferevent_, NetworkHandler::get().evdns(), AF_UNSPEC,
                host.c_str(), port) == 0)
        {
            link->initialized_ = true;
            return link;
...
TcpLink::TcpLink(Callbacks &callbacks)
    : callbacks_(callbacks), initialized_(false)
{
    bufferevent_ = bufferevent_socket_new(NetworkHandler::get().event(), -1,
                                          BEV_OPT_CLOSE_ON_FREE);
}


TcpLink::TcpLink(Callbacks &callbacks, evutil_socket_t sock)
    : callbacks_(callbacks), initialized_(true)
{
    bufferevent_ = bufferevent_socket_new(NetworkHandler::get().event(), sock,
                                          BEV_OPT_CLOSE_ON_FREE);
    bufferevent_setcb(bufferevent_, readCallback, writeCallback, eventCallback,
                      this);
