...
void TcpServer::onConnection(evconnlistener *listener, evutil_socket_t sock,
...
void TcpServer::onAcceptError(evconnlistener *listener, void *ptr)
{
    ...
    server->callbacks_.onError(
        evutil_socket_error_to_string(EVUTIL_SOCKET_ERROR()));
}


TcpServerPtr TcpServer::bind(Callbacks &callbacks,
                             const std::string &bindAddress)
{
    sockaddr_storage addr;
    int socklen = sizeof(sockaddr_storage);

    if (evutil_parse_sockaddr_port(bindAddress.c_str(),
    ...
    if ((server->listener_ = evconnlistener_new_bind(
             NetworkHandler::get().event(), onConnection, server.get(),
