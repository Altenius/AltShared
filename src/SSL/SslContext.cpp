...
bool SslContext::init(bool client, int transport, CtrDrbgContextPtr ctrDrbg,
                      const char *hostname)
{
    ctrDrbg_ = ctrDrbg;
    if (ctrDrbg_ == nullptr)
    ...
    if (mbedtls_ssl_config_defaults(
            &config_, client ? MBEDTLS_SSL_IS_CLIENT : MBEDTLS_SSL_IS_SERVER,
            transport, MBEDTLS_SSL_PRESET_DEFAULT) != 0)
    {
        return false;
    }
    mbedtls_ssl_conf_authmode(&config_, MBEDTLS_SSL_VERIFY_NONE);
    mbedtls_ssl_conf_rng(&config_, mbedtls_ctr_drbg_random,
                         ctrDrbg_->context());

    mbedtls_ssl_set_bio(&context_, this, &SslContext::sslSend,
                        &SslContext::sslReceive, NULL);
    if (hostname != nullptr)
    {
